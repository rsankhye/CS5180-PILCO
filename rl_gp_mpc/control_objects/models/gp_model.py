...
class GpStateTransitionModel(AbstractStateTransitionModel):
    ...
    def prepare_inference(self, inputs: torch.Tensor, state_changes: torch.Tensor):
        ...
        self.x_mem = inputs.double()
        self.y_mem = state_changes.double()
        self.iK, self.beta = calculate_factorizations(inputs, state_changes, self.models)
        self.iK.double() ; self.beta.double()
        # self.x_mem = inputs[:self.beta.shape[1]]
        self.lengthscales = torch.stack([model.covar_module.base_kernel.lengthscale[0] for model in self.models]).double()
        self.variances = torch.stack([model.covar_module.outputscale for model in self.models]).double()
        self.iL = torch.diag_embed(1 / self.lengthscales).double()
         
    @staticmethod
    def train(saved_state:SavedState,
