...
def sigmoid(x, umax = 1):
    """Saturates the input in a softmax/min manner"""
    e_x = umax/(1 + torch.exp(-x)) - umax/(1 + torch.exp(x))
    return e_x 

def sine_squash(x,umax=1):
    """Saturates the input in a sine squashed manner"""
    u = umax*torch.sin(x)
    return u
...
class RBFLayer(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        # Apply a sigmoid 
        if self.saturation =="sigmoid":
            output  = sigmoid((out.sum(dim=-1)),umax=self.umax)
        elif self.saturation == "sine":
            output  = sine_squash((out.sum(dim=-1)),umax=self.umax)
        else:
            print("Invalid saturation method defined. Options are sigmoid and sine.")
        return output #(out.sum(dim=-1))
